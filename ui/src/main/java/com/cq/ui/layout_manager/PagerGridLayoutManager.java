...
package com.cq.ui.layout_manager;
...
import static com.cq.ui.layout_manager.PagerConfig.Loge;
import static com.cq.ui.layout_manager.PagerConfig.Logi;
...
public class PagerGridLayoutManager extends RecyclerView.LayoutManager
    ...
    public boolean isLastPage(){
        int itemCount = getItemCount();//实际数量
        int onePageCount = mColumns * mRows;//一页的数量
        int currentPage = getCurrentPage();
        int fullCount = (currentPage+1)*onePageCount;//当前页总共排满的数量
        Loge("itemCount = "+itemCount+"--fullCount = "+fullCount);
        if (fullCount >= itemCount) return true;
        return false;
    }
}